...
**ALWAYS** run `git branch --show-current` fresh before creating a PR. Do NOT assume the branch is the same as earlier in the conversation — the user may have switched branches. If the current branch differs from what was discussed or seems unexpected, ask the user to confirm: "You are currently on branch `<branch>` — is this the correct branch to create a PR from?"
