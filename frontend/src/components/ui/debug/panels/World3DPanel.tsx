...
// Camera matches Mars game: phi=PI/2 (equator), slight tilt down
const CAM_PITCH = 0.3; // slight downward tilt
const COS_P = Math.cos(CAM_PITCH);
const SIN_P = Math.sin(CAM_PITCH);

// Project 3D world point to 2D screen (camera along +Z looking at origin, then tilted)
function project3D(x: number, y: number, z: number, cx: number, cy: number, scale: number) {
  // Camera rotation: tilt around X axis (look slightly down)
...
const SunPreview: React.FC<{ dirX: number; dirY: number; dirZ: number }> = ({
  ...
  const draw = useCallback(() => {
    ...
    // Build two perpendicular vectors to the light direction for the arrow grid
    let upX = 0,
      upY = 1,
      upZ = 0;
    if (Math.abs(ly) > 0.9) {
    ...
    const ax = t1x / t1Len,
      ay = t1y / t1Len,
      az = t1z / t1Len;
    // bitangent = light × tangent
    const bx = ly * az - lz * ay;
    const by = lz * ax - lx * az;
    const bz = lx * ay - ly * ax;

    // Generate arrow 3D positions: grid on a plane perpendicular to light, offset from sphere
    const arrowDist = 1.8;
    ...
    // Draw arrows BEHIND the sphere first, then sphere, then arrows in front
    ...
    // Separate arrows into behind and in-front based on depth
    ...
    // 2. Draw sphere with shading and anti-aliased edges
    ...
    for (let py = 0; py < h; py++) {
      for (let px = 0; px < w; px++) {
        ...
        // Alpha for anti-aliasing at edge
        let alpha = 1;
        if (r2 > 1) continue;
    ...
    // 3. Draw front arrows (bright, on top of sphere)
