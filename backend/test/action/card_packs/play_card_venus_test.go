...
func TestAerialMappers_PlayAndStorageCreated(t *testing.T) {
	...
	p.Hand().AddCard("card-aerial-mappers-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 11}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-aerial-mappers-test", payment, nil, nil, nil, nil)
	testutil.AssertNoError(t, err, "Aerial Mappers should play successfully")

	storage := p.Resources().GetCardStorage("card-aerial-mappers-test")
...
func TestAerosportTournament_RequiresFloaters(t *testing.T) {
	...
	floaterRes := shared.ResourceFloater
	card := gamecards.Card{
		ID:   "card-aerosport-tournament-test",
		...
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementResource, Min: testutil.IntPtr(5), Resource: &floaterRes},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
				...
				Outputs: []shared.ResourceCondition{
					{ResourceType: shared.ResourceCredit, Amount: 1, Target: "self-player"},
				},
			},
		},
	}
	...
	p.Hand().AddCard("card-aerosport-tournament-test")

	// Player has no floaters - should fail
	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 7}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-aerosport-tournament-test", payment, nil, nil, nil, nil)
...
func TestAtalantaPlanitiaLab_RequiresScienceTags(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-atalanta-planitia-lab-test",
		Name: "Atalanta Planitia Lab",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 10,
		Tags: []shared.CardTag{shared.TagScience, shared.TagVenus},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(3), Tag: testutil.TagPtr(shared.TagScience)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
	...
	p.Hand().AddCard("card-atalanta-planitia-lab-test")

	// Player has 0 science tags - should fail
	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 10}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-atalanta-planitia-lab-test", payment, nil, nil, nil, nil)
...
func TestAtalantaPlanitiaLab_SucceedsWithScienceTags(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, sciCard1, sciCard2, sciCard3}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
...
func TestCorroderSuits_CreditProductionIncrease(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, venusTargetCard}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-corroder-suits-test")
	...
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-corroder-suits-test", payment, nil, []string{targetCardID}, nil, nil)
...
func TestDeuteriumExport_PlayAndStorageCreated(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-deuterium-export-test",
		Name: "Deuterium Export",
		Type: gamecards.CardTypeActive,
		Pack: "venus-next",
		Cost: 11,
		Tags: []shared.CardTag{shared.TagPower, shared.TagSpace, shared.TagVenus},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeManual}},
				Choices: []shared.Choice{
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-deuterium-export-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 11}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-deuterium-export-test", payment, nil, nil, nil, nil)
	testutil.AssertNoError(t, err, "Deuterium Export should play successfully")

	storage := p.Resources().GetCardStorage("card-deuterium-export-test")
...
func TestExtractorBalloons_PlayAdds3Floaters(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-extractor-balloons-test",
		...
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeAuto}},
				Outputs: []shared.ResourceCondition{
					{ResourceType: shared.ResourceFloater, Amount: 3, Target: "self-card"},
				},
			},
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeManual}},
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-extractor-balloons-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 21}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-extractor-balloons-test", payment, nil, nil, nil, nil)
	testutil.AssertNoError(t, err, "Extractor Balloons should play successfully")

	storage := p.Resources().GetCardStorage("card-extractor-balloons-test")
...
func TestExtremophiles_RequiresScienceTags(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-extremophiles-test",
		Name: "Extremophiles",
		Type: gamecards.CardTypeActive,
		Pack: "venus-next",
		Cost: 3,
		Tags: []shared.CardTag{shared.TagMicrobe, shared.TagVenus},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-extremophiles-test")

	// No science tags - should fail
	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 3}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-extremophiles-test", payment, nil, nil, nil, nil)
...
func TestGHGImportFromVenus_HeatProductionIncrease(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-ghg-import-test",
		Name: "GHG Import From Venus",
		Type: gamecards.CardTypeEvent,
		Pack: "venus-next",
		Cost: 23,
		Tags: []shared.CardTag{shared.TagSpace, shared.TagVenus},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeAuto}},
				Outputs: []shared.ResourceCondition{
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-ghg-import-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 23}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-ghg-import-test", payment, nil, nil, nil, nil)
...
func TestIoSulphurResearch_DrawOneCardWithoutVenusTags(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-io-sulphur-test",
		Name: "Io Sulphur Research",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 17,
		Tags: []shared.CardTag{shared.TagJovian, shared.TagScience},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeAuto}},
				Choices: []shared.Choice{
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-io-sulphur-test")
	...
	choiceIndex := 0
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-io-sulphur-test", payment, &choiceIndex, nil, nil, nil)
...
func TestIoSulphurResearch_FailsDraw3WithoutVenusTags(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-io-sulphur-fail-test")

	// No venus tags - choice 1 (draw 3) should fail
	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 17}
	choiceIndex := 1
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-io-sulphur-fail-test", payment, &choiceIndex, nil, nil, nil)
...
func TestIshtarMining_TitaniumProductionIncrease(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-ishtar-mining-test")
	...
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-ishtar-mining-test", payment, nil, nil, nil, nil)
...
func TestLunaMetropolis_CreditProductionPerEarthTag(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-luna-metropolis-test",
		Name: "Luna Metropolis",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 21,
		Tags: []shared.CardTag{shared.TagCity, shared.TagEarth, shared.TagSpace},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeAuto}},
				Outputs: []shared.ResourceCondition{
	...
	p.Hand().AddCard("card-luna-metropolis-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 21}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-luna-metropolis-test", payment, nil, nil, nil, nil)
...
func TestLuxuryFoods_FailsWithoutRequiredTags(t *testing.T) {
	...
	card := gamecards.Card{
		...
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(1), Tag: testutil.TagPtr(shared.TagEarth)},
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(1), Tag: testutil.TagPtr(shared.TagJovian)},
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(1), Tag: testutil.TagPtr(shared.TagVenus)},
			},
		},
	}
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-luxury-foods-test")

	// No tags at all - should fail
	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 8}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-luxury-foods-test", payment, nil, nil, nil, nil)
...
func TestLuxuryFoods_SucceedsWithAllTags(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-luxury-foods-success-test",
		Name: "Luxury Foods",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 8,
	...
	additionalCards := []gamecards.Card{card, earthCard, jovianCard, venusCard}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Hand().AddCard("card-luxury-foods-success-test")

	creditsBefore := p.Resources().Get().Credits

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 8}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-luxury-foods-success-test", payment, nil, nil, nil, nil)
...
func TestGyropolis_ProductionAndCityPlacement(t *testing.T) {
	...
	anywhereLocation := "anywhere"
	earthTag := shared.TagEarth
	venusTag := shared.TagVenus

	card := gamecards.Card{
		ID:   "card-gyropolis-test",
		Name: "Gyropolis",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 20,
	...
	additionalCards := []gamecards.Card{card, earthCard, venusCard1, venusCard2}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Hand().AddCard("card-gyropolis-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 20}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-gyropolis-test", payment, nil, nil, nil, nil)
...
func TestMiningQuota_IncreaseSteelProduction(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, earthHelper, jovianHelper, venusHelper}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-mining-quota-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 5}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-mining-quota-test", payment, nil, nil, nil, nil)
...
func TestMiningQuota_FailsWithoutRequiredTags(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-mining-quota-fail-test",
		Name: "Mining Quota",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 5,
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-mining-quota-fail-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 5}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-mining-quota-fail-test", payment, nil, nil, nil, nil)
...
func TestNeutralizerFactory_IncreaseVenus(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-neutralizer-factory-test",
...
func TestOmnicourt_IncreaseTR(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-omnicourt-test")

	trBefore := p.Resources().TerraformRating()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 11}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-omnicourt-test", payment, nil, nil, nil, nil)
...
func TestOrbitalReflectors_HeatProductionAndVenus(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-orbital-reflectors-test",
		...
		Behaviors: []shared.CardBehavior{
			{
				...
				Outputs: []shared.ResourceCondition{
					...
					{ResourceType: shared.ResourceVenus, Amount: 2, Target: "none"},
				},
			},
		},
	}
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-orbital-reflectors-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 26}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-orbital-reflectors-test", payment, nil, nil, nil, nil)
...
func TestSisterPlanetSupport_CreditProduction(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-sister-planet-support-test",
		Name: "Sister Planet Support",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 7,
		Tags: []shared.CardTag{shared.TagVenus, shared.TagEarth},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(1), Tag: testutil.TagPtr(shared.TagVenus)},
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(1), Tag: testutil.TagPtr(shared.TagEarth)},
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-sister-planet-support-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 7}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-sister-planet-support-test", payment, nil, nil, nil, nil)
...
func TestSolarnet_Draw2Cards(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, venusHelper, earthHelper, jovianHelper}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-solarnet-test")

	handSizeBefore := len(p.Hand().Cards())

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 7}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-solarnet-test", payment, nil, nil, nil, nil)
...
func TestSpinInducingAsteroid_RaiseVenus(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-spin-inducing-asteroid-test",
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-spin-inducing-asteroid-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 16}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-spin-inducing-asteroid-test", payment, nil, nil, nil, nil)
...
func TestSponsoredAcademies_DiscardAndDraw(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-sponsored-academies-test",
		Name: "Sponsored Academies",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 9,
		Tags: []shared.CardTag{shared.TagEarth, shared.TagScience},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeAuto}},
				Outputs: []shared.ResourceCondition{
					{ResourceType: shared.ResourceCardDiscard, Amount: 1, Target: "self-player"},
					{ResourceType: shared.ResourceCardDraw, Amount: 3, Target: "self-player"},
					{ResourceType: shared.ResourceCardDraw, Amount: 1, Target: "other-player"},
				},
			},
		},
	}
	...
	p := players[0]
	p.SetCorporationID("corp-tharsis-republic")
	players[1].SetCorporationID("corp-tharsis-republic")
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-sponsored-academies-test")
	p.Hand().AddCard("card-discard-fodder")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 9}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-sponsored-academies-test", payment, nil, nil, nil, nil)
...
func TestSulphurExports_CreditProductionPerVenusTag(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, venusHelper1, venusHelper2}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-sulphur-exports-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 21}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-sulphur-exports-test", payment, nil, nil, nil, nil)
...
func TestTerraformingContract_CreditProduction(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-terraforming-contract-test",
		...
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementTR, Min: testutil.IntPtr(25)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-terraforming-contract-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 8}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-terraforming-contract-test", payment, nil, nil, nil, nil)
...
func TestTerraformingContract_FailsBelowTR25(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-terraforming-contract-fail-test",
		Name: "Terraforming Contract",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 8,
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-terraforming-contract-fail-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 8}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-terraforming-contract-fail-test", payment, nil, nil, nil, nil)
...
func TestWaterToVenus_RaiseVenus(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-water-to-venus-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 9}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-water-to-venus-test", payment, nil, nil, nil, nil)
...
func TestVenusGovernor_CreditProduction(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-venus-governor-test",
		Name: "Venus Governor",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 4,
		Tags: []shared.CardTag{shared.TagVenus, shared.TagVenus},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementTags, Min: testutil.IntPtr(2), Tag: testutil.TagPtr(shared.TagVenus)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
	...
	// Need 2 venus tags before playing (card has 2 venus tags but requires 2 already)
	p.PlayedCards().AddCard("venus-1", "Venus Card 1", "automated", []string{"venus"})
	p.PlayedCards().AddCard("venus-2", "Venus Card 2", "automated", []string{"venus"})

	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-venus-governor-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 4}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-venus-governor-test", payment, nil, nil, nil, nil)
	testutil.AssertNoError(t, err, "Venus Governor should play with 2+ venus tags")
...
func TestVenusSoils_PlantProductionAndVenus(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-venus-soils-test")

	prodBefore := p.Resources().Production()

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 20}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-venus-soils-test", payment, nil, nil, nil, nil)
...
func TestVenusWaystation_DiscountEffect(t *testing.T) {
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-venus-waystation-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 9}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-venus-waystation-test", payment, nil, nil, nil, nil)
...
func TestMaxwellBase_DecreaseEnergyProductionAndCityPlacement(t *testing.T) {
	...
	card := gamecards.Card{
		...
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementVenus, Min: testutil.IntPtr(12)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
				...
				Outputs: []shared.ResourceCondition{
					{ResourceType: shared.ResourceEnergyProduction, Amount: -1, Target: "self-player"},
					{ResourceType: shared.ResourceCityPlacement, Amount: 1, Target: "none",
						TileRestrictions: &shared.TileRestrictions{BoardTags: []string{"maxwell-base"}}},
				},
			},
			{
				Triggers: []shared.Trigger{{Type: shared.TriggerTypeManual}},
				Outputs: []shared.ResourceCondition{
					{
						ResourceType: shared.ResourceCardResource,
						Amount:       1,
						Target:       "any-card",
						Selectors:    []shared.Selector{{Tags: []shared.CardTag{shared.TagVenus}}},
					},
...
func TestStratopolis_CityPlacement(t *testing.T) {
	...
	additionalCards := []gamecards.Card{card, sciHelper1, sciHelper2}
	cardRegistry := testutil.CreateTestCardRegistryWithAdditionalCards(additionalCards)
	...
	p.Resources().Add(map[shared.ResourceType]int{shared.ResourceCredit: 100})
	p.Hand().AddCard("card-stratopolis-test")

	playCardAction := cardAction.NewPlayCardAction(repo, cardRegistry, nil, logger)
	payment := cardAction.PaymentRequest{Credits: 22}
	err := playCardAction.Execute(ctx, testGame.ID(), p.ID(), "card-stratopolis-test", payment, nil, nil, nil, nil)
...
func TestVenusianAnimals_PlaysAndRegistersEffect(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-venusian-animals-test",
		Name: "Venusian Animals",
		Type: gamecards.CardTypeActive,
		Pack: "venus-next",
		Cost: 15,
		Tags: []shared.CardTag{shared.TagAnimal, shared.TagScience, shared.TagVenus},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementVenus, Min: testutil.IntPtr(18)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
				Triggers: []shared.Trigger{
					{
						...
						Condition: &shared.ResourceTriggerCondition{
							Type:      "tag-played",
							Target:    func() *string { s := "self-player"; return &s }(),
...
func TestVenusianPlants_RaiseVenusWithAnimalChoice(t *testing.T) {
	...
	card := gamecards.Card{
		ID:   "card-venusian-plants-test",
		Name: "Venusian Plants",
		Type: gamecards.CardTypeAutomated,
		Pack: "venus-next",
		Cost: 13,
		Tags: []shared.CardTag{shared.TagPlant, shared.TagVenus},
		Requirements: &gamecards.CardRequirements{
			Items: []gamecards.Requirement{
				{Type: gamecards.RequirementVenus, Min: testutil.IntPtr(16)},
			},
		},
		Behaviors: []shared.CardBehavior{
			{
