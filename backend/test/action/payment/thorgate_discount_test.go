...
func TestThorGate_CardDiscount(t *testing.T) {
	...
	deepWellHeatingCard, err := cardRegistry.GetByID(testutil.CardID("Deep Well Heating"))
...
func TestThorGate_CombinedDiscount(t *testing.T) {
	...
	// Find the discount behavior (the one with auto trigger and discount output)
	var discountBehavior shared.CardBehavior
	var discountBehaviorIndex int
	for i, b := range thorGateCard.Behaviors {
		if gamecards.HasAutoTrigger(b) && gamecards.HasPersistentEffects(b) {
			discountBehavior = b
			discountBehaviorIndex = i
			break
		}
	}
...
func TestDiscountORLogic(t *testing.T) {
	...
	asteroidCard, err := cardRegistry.GetByID(testutil.CardID("Asteroid"))
